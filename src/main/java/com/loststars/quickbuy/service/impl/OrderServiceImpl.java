...
//            orderModel.setItemPrice(itemModel.getPrice());
//        }
        orderModel.setId(SpringApplicationContextUtil.getApplicationContext().getBean(this.getClass()).createOrderId());
        orderModel.setOrderPrice(orderModel.getItemPrice().multiply(new BigDecimal(orderModel.getAmount())));
        if (! itemService.decreaseStock(itemId, amount)) throw new BusinessException(EmBusinessError.ITEM_STOCK_LIMIT);
        orderDOMapper.insertSelective(convertFromModel(orderModel));
        StockLogDO stockLogDO = new StockLogDO();
        stockLogDO.setId(stockLogId);
        stockLogDO.setStatus(StockLogDO.STATUS_COMMIT);
        stockLogDOMapper.updateByPrimaryKeySelective(stockLogDO);
        if (! itemService.increaseSales(itemId, amount)) throw new BusinessException(EmBusinessError.PARAMETER_VALIDATION_ERROR, "更新销量错误");
        return orderModel;
    }
